👋 Olá, eu sou Nickolas Sanhudo Grundemann (Nick)
---
Transformando lógica em software — uma linha de código, uma partida, uma vez por vez
Atuo conectando lógica de programação, ciclo de vida de software e otimização de performance para construir minha base como desenvolvedor e impulsionar minha entrada no mercado de tecnologia.
Sou estudante do Técnico em Desenvolvimento de Sistemas (TDS) no Senac Tech. Minha jornada une raciocínio lógico, disciplina de treino competitivo e curiosidade técnica, sempre com foco em conquistar uma carreira estável em desenvolvimento.

---

## 🚀 Sobre mim

- 🎓 Técnico em Desenvolvimento de Sistemas (TDS) — Senac
- 💻 Base em lógica de programação, HTML e Python; JavaScript em consolidação - Alura 
- 🤖 Entusiasta e aplicador de Inteligência Artificial
- 📊 Apaixonado por Disciplina, consistência, dados, inovação e transformação digital
- 🌱 Aprendizado contínuo em desenvolvimento de sistemas e inglês
- 🎯 Focado em criar soluções que conectem pessoas, conhecimento e tecnologia

---

## 🛠️ Stack & Ferramentas

### 🚀 Usos no dia a dia

<div align="left">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" title="Python"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45" title="JavaScript"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" title="HTML5"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45" title="CSS3"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" title="GitHub"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45" title="VS Code"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/notion/notion-original.svg" width="45" title="Notion"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" width="45" title="Figma"/>

</div>

---

## 🤖 Ferramentas & Ecosistema

<div align="left">

| Ferramenta           | Aplicação                                              |
| -------------------- | ------------------------------------------------------ |
| ChatGPT              | Pesquisa, automação, geração de conteúdo e programação |
| Claude               | Análise, escrita estratégica e documentação            |
| Canva                | Design, apresentações e comunicação visual             |
| Notion               | Organização de projetos e gestão do conhecimento       |
| IA Generativa        | Educação, produtividade e inovação                     |

</div>

---

## 🌱 Em evolução

<div align="left">
  
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" title="Python"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45" title="JavaScript"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" title="HTML5"/>

</div>

---

## 💻 Ambiente

<div align="left">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/windows11/windows11-original.svg" width="45" title="Windows 11"/>

</div>

---

## 🎯 Áreas de Foco

* Inteligência Artificial
* Desenvolvimento Web
* Análise de Dados
* Automação de Processos
* Gestão de Projetos
* Inovação e Transformação Digital

---

## 📫 Vamos conectar?
 <div>
   <a href="mailto:nick.grundemann@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</div>
 </div>
     
 ---
 
 > **"A tecnologia por si só não transforma o mundo. Pessoas que aprendem, inovam e compartilham conhecimento, sim."** 🚀
